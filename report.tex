...
A Wilcoxon signed-rank test indicated that for units in the \texttt{VISam} region of the visual cortex, the average spike rate over static presentations was higher than over drifting presentations (Figure \ref{fig:spike_mean}, \(z=0.0\), \(p<.001\)), and the coefficient of variation lower (Figure \ref{fig:spike_cv}, \(z=16.0\), \(p<.001\)).
...
where \( r_\alpha \) is the average firing rate of a unit over presentations with orientation \( \alpha \), \( \theta^* = \arg\max_\theta r_\theta \) is the value which maximizes the firing rate, and \( \bar{\theta} \) is the angle orthogonal to \( \theta^* \).
